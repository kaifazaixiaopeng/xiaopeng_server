...
public class SortUtil<T> {
    // 按任意属性进行排序
    public static class AnyProperComparator<T> implements Comparator<T> {

        private String properName;// 根据此关键字属性排序

        private boolean flag;// 为false的时候是正序，为true的时候是倒序
        ...
        public int compare(Object r1, Object r2) {
            ...
            try {
                ...
                if ("String".equals(classType)) {
                    ...
                    method = c.getMethod("get" + properName.substring(0, 1).toUpperCase() + properName.substring(1), new Class[]{});
                    BigDecimal bd1 = (BigDecimal) method.invoke(r1);
                    BigDecimal bd2 = (BigDecimal) method.invoke(r2);
                    if (!flag) {
                        result = bd1.compareTo(bd2);
                    } else {
                        result = bd2.compareTo(bd1);
                    }
                } else {
    ...
    public static <T> List<T> anyProperSort(List<T> list, String properName, boolean flag) {
        AnyProperComparator<T> comparator = new AnyProperComparator<>(properName, flag);
